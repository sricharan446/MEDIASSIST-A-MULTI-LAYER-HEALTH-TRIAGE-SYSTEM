...
class TestEmergencyDetector:
    ...
    def test_emergency_message_triggers_emergency_response(self, client, message):
        ...
        with patch("app.load_users", new_callable=AsyncMock, return_value={VALID_USER: {"token": VALID_TOKEN}}), \
             patch("app.memory") as mock_mem, \
             patch("app.agent") as mock_agent:
            mock_mem.create_session.return_value = TEST_SESSION
            mock_mem.get_history.return_value = []
            mock_mem.load_profile.return_value = {}
            mock_mem.load_report.return_value  = None
            mock_mem.set_session_name.return_value = None
            mock_mem.add_message.return_value = None
            mock_agent.process = AsyncMock(return_value={"response": "Emergency help", "tools_used": ["Emergency Detector"]})
    ...
    def test_non_emergency_message_skips_emergency_layer(self, client):
        ...
        with patch("app.load_users", new_callable=AsyncMock, return_value={VALID_USER: {"token": VALID_TOKEN}}), \
            ...
            mock_mem.set_session_name.return_value = None
            mock_mem.add_message.return_value = None
            mock_agent.process = AsyncMock(return_value={"response": "Normal response", "tools_used": []})
...
class TestSymptomPredictor:
    ...
    SYMPTOM_MESSAGES = [
        ...
        "mild back pain and muscle ache",
    ]
    ...
    def test_symptom_messages_trigger_symptom_predictor(self, client, message):
        ...
        with patch("app.load_users", new_callable=AsyncMock, return_value={VALID_USER: {"token": VALID_TOKEN}}), \
            ...
            mock_mem.set_session_name.return_value = None
            mock_mem.add_message.return_value = None
            mock_agent.process = AsyncMock(return_value={"response": "Symptom analysis", "tools_used": ["Symptom Predictor"]})
